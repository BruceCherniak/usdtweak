...
void PreferencesModalDialog::Draw() {
    ...
    ImVec2 prefSize(0, RemainingHeight(2));
    ImGui::PushItemWidth(g.FontSize * 5); // heuristic
    ImGui::ListBox("##PreferencePanels", &current_item, panels, 3);
    ImGui::SameLine();
    if (current_item == 0) {
        ImGui::BeginChild("##General", prefSize);
        ...
        ImGui::BeginChild("##Viewport", prefSize);
        bool texOn;
        ImGui::Checkbox("Texture On by default", &texOn);
        ImGui::EndChild();
    }
    else if (current_item == 2) {
        ImGui::BeginChild("##Style", prefSize);
